  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-6xl px-4 py-12 sm:py-16 sm:px-6 lg:px-8">
        ...
        <div className="space-y-14">
          {filteredDays.map((day, dayIndex) => (
            <div key={day.id} className="space-y-6">
              {/* Day Header Banner */}
              <div className="overflow-hidden rounded-xl border border-[#002F6C] bg-gradient-to-r from-navy-950 via-navy-900 to-navy-950 p-6 text-white shadow-md">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <span className="flex h-10 w-10 items-center justify-center rounded-lg bg-gold font-mono text-base font-extrabold text-navy-950 shadow-sm">
                      0{dayIndex + 1}
                    </span>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="rounded bg-white/15 px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider text-gold-300">
                          {day.badge}
                        </span>
                  ...
                  <div className="flex items-center gap-2 text-xs text-white/70 bg-white/10 px-3 py-1.5 rounded-lg w-fit">
                    <Clock size={14} className="text-gold" />
                    <span>{day.items.length} Scheduled Sessions</span>
                  </div>
                </div>
                <p className="mt-3 text-xs sm:text-sm text-white/80 border-t border-white/10 pt-3">
                  {day.focus}
                </p>
              </div>
                  ...
                  return (
                    <Reveal key={session.title + session.time} delay={sessionIdx * 0.04}>
                      <div className="relative flex items-start gap-4 sm:gap-6 group">
                        ...
                        <div
                          className={cn(
                            "relative z-10 flex h-8 w-8 sm:h-9 sm:w-9 shrink-0 items-center justify-center rounded-full border-2 transition-transform duration-300 group-hover:scale-110 shadow-xs",
                            session.highlight
                              ? "border-gold bg-navy-900 text-gold"
                              : "border-gray-300 bg-white text-navy-900 group-hover:border-navy"
                          )}
                        >
                        ...
                        <div
                          className={cn(
                            "flex-1 rounded-xl border bg-white p-5 sm:p-6 shadow-sm transition-all duration-300 hover:shadow-md",
                            session.highlight
                              ? "border-navy/40 ring-1 ring-navy/10 hover:border-navy"
                        ...
                        >
                          ...
                          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 pb-3 mb-3">
                            <div className="flex flex-wrap items-center gap-2">
                              <span
                                className={cn(
                                  "rounded border px-2.5 py-0.5 text-xs font-bold uppercase tracking-wider",
                            ...
                            <div className="flex items-center gap-2">
                              <span className="inline-flex items-center gap-1 font-mono text-xs font-bold text-navy-950 bg-navy-50/80 px-2.5 py-1 rounded-md border border-navy-100">
                          ...
                          <h4 className="text-base sm:text-lg font-bold text-navy-950 tracking-tight group-hover:text-navy transition-colors">
                            {session.title}
                          </h4>
        ...
        <Reveal delay={0.1}>
          <div className="mt-12 rounded-xl border border-[#E5E7EB] bg-white p-6 sm:p-8 shadow-sm">
            <div className="grid gap-6 lg:grid-cols-12 lg:items-center">
              <div className="lg:col-span-8">
                <span className="inline-block rounded bg-gold-50 px-2.5 py-0.5 text-xs font-bold uppercase tracking-wider text-gold-900 mb-2">
